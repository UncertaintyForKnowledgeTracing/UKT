...
def init_test_datasets(data_config, model_name, batch_size, diff_level=None):
    ...
    if  model_name in que_type_models:
        ...
        test_dataset = UKTDataset(os.path.join(data_config["dpath"], data_config["test_file"]), data_config["input_type"], {-1},need_aug=False)
        test_window_dataset = UKTDataset(os.path.join(data_config["dpath"], data_config["test_window_file"]), data_config["input_type"], {-1},need_aug=False)
        ...
        if "test_question_file" in data_config:
            test_question_dataset = UKTDataset(os.path.join(data_config["dpath"], data_config["test_question_file"]), data_config["input_type"], {-1}, qtest=True,need_aug=False)
            test_question_window_dataset = UKTDataset(os.path.join(data_config["dpath"], data_config["test_question_window_file"]), data_config["input_type"], {-1}, qtest=True,need_aug=False)
    else:
